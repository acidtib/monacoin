...
def setup():
    ...
    if not os.path.isdir('monacoin'):
        subprocess.check_call(['git', 'clone', 'https://github.com/arubinofaux/devdev.git'])
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs.mona/', '-r', args.version+'-linux', '../monacoin/contrib/gitian-descriptors/gitian-linux.yml']):
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/arubinofaux/devdev', help='Specify the URL of the repository. Default is %(default)s')
